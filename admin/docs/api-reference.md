...
| GET    | `/api/system/offline-status`          | Check MONAD offline readiness, local AI, vault, and vector cache |
...
MONAD can export selected data as plain Markdown into a local Obsidian-compatible vault. The vault path is configured with `VAULT_PATH` and defaults to `storage/vault`.

| Method | Path                         | Description                                                     |
| ------ | ---------------------------- | --------------------------------------------------------------- |
| GET    | `/api/vault/status`          | Check vault path, writability, and configured folders           |
| GET    | `/api/vault/index/status`    | Check the local vector index status                             |
| POST   | `/api/vault/index`           | Refresh or rebuild the local vault vector index                 |
| POST   | `/api/vault/save`            | Save Markdown into `notes`, `docs`, `ai`, or `services`         |
| GET    | `/api/vault/list`            | List Markdown files, optionally filtered by `folder`            |
| GET    | `/api/vault/search`          | Search vault Markdown content by local text query               |
| POST   | `/api/vault/search/semantic` | Search vault Markdown using local embeddings                    |
| POST   | `/api/vault/ask`             | Answer a question using vault content only, with file citations |

See the [Obsidian Vault guide](/docs/obsidian-vault) for folder structure and Markdown frontmatter rules.
