...
MONAD (Node for Offline Media, Archives, and Data) is a personal server that gives you access to knowledge, education, and AI assistance without requiring an internet connection. It runs on your own hardware, keeping your data private and accessible anytime.

### Do I need internet to use MONAD?
...
MONAD is designed for capable hardware, especially if you want to use the AI features. Recommended:
...
For best results, use hardware with enough storage for your content library and a GPU if you plan to run local AI models heavily.
...
For Kiwix content, MONAD uses standard ZIM files. For educational content, Kolibri uses its own channel format.
...
MONAD documentation is automatically added to the Knowledge Base when the AI Assistant is installed.

### What is the System Benchmark?

The System Benchmark tests your hardware performance and generates a MONAD Score — a weighted composite of CPU, memory, disk, and AI performance. You can create a Builder Tag and keep results locally for comparison.
...
MONAD automatically detects NVIDIA GPUs when the NVIDIA Container Toolkit is installed on the host system. To set up GPU acceleration:
...
MONAD will detect the GPU during installation and configure the AI to use it automatically. You'll see "NVIDIA container runtime detected" in the installation progress.
...
When you add or swap a GPU, MONAD needs to reconfigure the AI container to use it:
...
MONAD checks whether your GPU is actually accessible inside the AI container. If a GPU is detected on the host but isn't working inside the container, you'll see a warning banner on the System Information and AI Settings pages. Click the **"Fix: Reinstall AI Assistant"** button to recreate the container with proper GPU access. This preserves your downloaded AI models.
...
For advanced troubleshooting or when you can't access the web interface, MONAD includes helper scripts under `install/` in the project folder:

**Start all services:**

```bash
sudo bash install/start_monad.sh
```

**Stop all services:**

```bash
sudo bash install/stop_monad.sh
```

**Update Command Center:**

```bash
sudo bash install/update_monad.sh
```

_Note: This updates the Command Center only, not individual apps. Update apps through the web interface._

**Uninstall MONAD:**

```bash
curl -fsSL https://raw.githubusercontent.com/seclib/monad/refs/heads/main/install/uninstall_monad.sh -o uninstall_monad.sh
sudo bash uninstall_monad.sh
```

_Warning: This cannot be undone. All data will be deleted._
...
Yes. MONAD runs entirely on your hardware. Your searches, AI conversations, and usage data never leave your server.

### Can others access my server?

By default, MONAD is accessible on your local network. Anyone on the same network can access it. For public networks, consider additional security measures.
...
Try asking a question in [AI Chat](/chat). The local AI can answer questions about many topics, including technical troubleshooting. If you've uploaded MONAD documentation to the Knowledge Base, it can also help with MONAD-specific questions.
...
Report issues or suggest improvements on the **[MONAD GitHub repository](https://github.com/seclib/monad/issues)**.
