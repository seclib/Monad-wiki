...
The System Benchmark tests your hardware performance and generates a MONAD Score — a weighted composite of CPU, memory, disk, and AI performance. You can create a Builder Tag (a MONAD-themed identity like "Tactical-Llama-1234") and share your results with the [community leaderboard](https://benchmark.projectnomad.us).
...
For advanced troubleshooting or when you can't access the web interface, MONAD includes helper scripts in `/opt/monad`:

**Start all services:**
```bash
sudo bash /opt/monad/start_monad.sh
```

**Stop all services:**
```bash
sudo bash /opt/monad/stop_monad.sh
```

**Update Command Center:**
```bash
sudo bash /opt/monad/update_monad.sh
```
...
### Join the community
Get help from other MONAD users on **[Discord](https://discord.com/invite/crosstalksolutions)**.
